...
def evaluate_for_k(full_df: pd.DataFrame, holes_df: pd.DataFrame,
                   columns: List[str], k: int, int_columns: List[str]) -> Tuple[float, pd.DataFrame]:
    ...
    for r, cpos in zip(rows, cols):
        ...
        if not np.isfinite(y_pred):
            continue
        # Округляем для целочисленных атрибутов
        if col in int_columns:
            y_pred = float(np.rint(y_pred))
...
def main():
    ...
    ap.add_argument("--int-columns", default="fio,passport,seat_number,coach_number,train_number")
    ...
    int_columns = [c.strip() for c in (args.int_columns or "").split(",") if c.strip()]
    int_columns = [c for c in int_columns if c in holes_df.columns]
    ...
    for k in ks:
        total_percent, preds_df = evaluate_for_k(full_df, holes_df, columns, k, int_columns)
